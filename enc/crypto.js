...
function generateAKey() {
  ...
  window.crypto.subtle
    .generateKey(
      { name: "AES-GCM", length: 256 }, // Algorithm using this key
      ...
      window.crypto.subtle
        ...
        .then(function (aesKeyBuffer) {
          document.getElementById("aes-key").value =
            arrayBufferToHexString(aesKeyBuffer);
        })
        .catch(function (err) {
...
function encryptTheFile() {
  ...
  // Grab the key bytes
  const aesKeyBytes = hexStringToByteArray(
    document.getElementById("aes-key").value
  );
  ...
  reader.onload = function () {
    ...
    window.crypto.subtle
      ...
      .then(function (result) {
        var blob = new Blob([iv, new Uint8Array(result)], {
  ...
  window.crypto.subtle
    .importKey("raw", aesKeyBytes, { name: "AES-GCM", length: 256 }, true, [
      "encrypt",
      "decrypt",
    ])
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    ...
    .addEventListener("click", generateAKey);
